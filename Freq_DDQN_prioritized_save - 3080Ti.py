# -*- coding: utf-8 -*-
# 2022.3.21 action : core  and memory frequency
#
import math
import os
import numpy as np
...
# 恢复默认
# core 频率恢复默认
os.system("echo Sjy2022 | sudo -S nvidia-smi -rgc")
# GPU 内存频率恢复默认
os.system("echo Sjy2022 | sudo -S python3 ResetMemClocks.py")

# os.system("echo Sjy2022 | sudo -S nvidia-smi -pm 1")
# os.system("echo Sjy2022 | sudo -S nvidia-smi -pl 350")

# 强化学习 State：(1)GPU频率，(2)GPU利用率，(3)显存利用率，(4)实时功耗，(4)实时温度，(6)显存频率，(7)当前的power cap
# SM frequency NVIDIA 2080Ti 300~2100 15MHz   120个
# SM frequency NVIDIA 3080Ti 240~2130 15MHz   126 个
...
MINS = {'CLOCKS_GPU': 210, 'UTIL_GPU': 0, 'UTIL_MEM': 0, 'POWER': 30, 'TEMP': 30}
MAXS = {'CLOCKS_GPU': 1910, 'UTIL_GPU': 100, 'UTIL_MEM': 100, 'POWER': 350, 'TEMP': 90}
BUCKETS = {'CLOCKS_GPU': 128, 'UTIL_GPU': 10, 'UTIL_MEM': 10, 'POWER': 16, 'TEMP': 30}
...
# GPU power limit   GeForce RTX 3080 Ti  100~385 TDP:350
POWER_IN_STATE = 1 # 是否将功率上限作为一种 State
gpu_limit = (9501,1910)
fre_ran=range(1625, 1925, 15)
GPU = []
for mem_freq in 9501, 9251, 5001, 810, 405:
    if mem_freq == 9501:
        for gpu_freq in fre_ran:  #240, 2145, 15
            print(gpu_freq)
            GPU.append((mem_freq, gpu_freq))
    if mem_freq == 9251:
        for gpu_freq in fre_ran: #240, 2145, 15
            GPU.append((mem_freq, gpu_freq))
    # if mem_freq == 5001:
    ...
    #         GPU.append((mem_freq, gpu_freq))
    # if mem_freq == 810:
    #     for gpu_freq in  range(1185, 2115, 15): #210, 2115, 15
    #         GPU.append((mem_freq, gpu_freq))
    # if mem_freq == 405:
    #     for gpu_freq in  range(210, 435, 15):
    #         GPU.append((mem_freq, gpu_freq))
print(GPU)
...
# Memory frequency  NVIDIA GeForce RTX 3080 Ti  405,810,,5001,9251,9501
# 将内存频率作为一种 State
MEM =[405,810,5001,9251,9501]
#MEM =[9251,9501]
...
for step in range(STEPS):
    ...
    # take action
    os.system("echo Sjy2022 | sudo -S python3 SetMemClocks.py %s" %(gpu_limit[0]))
    os.system( "echo Sjy2022 | sudo -S nvidia-smi -lgc 210,%s" %(gpu_limit[1]))
...
save_path = saver.save(sess, '/home/sjy/project/cpu_gpu/RL/net-freq/save_net.ckpt')
print('Save to path: ', save_path)
# 存储神经网络框架
model_path = saver.save(sess, '/home/sjy/project/cpu_gpu/RL/net-freq/model')
